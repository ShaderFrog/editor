// From https://github.com/playcanvas/model-viewer/blob/61e8f51207dd8e7a676e1d55cb70e894fa20e337/src/orbit-camera.ts
import {
  EVENT_MOUSEDOWN,
  EVENT_MOUSEUP,
  EVENT_MOUSEMOVE,
...
class OrbitCamera {
  ...
  constructor(cameraNode: Entity, transitionTime: number) {
    ...
    this.azimElevDistance = new SmoothedValue(
      new Vec3(0, 0, 0),
      transitionTime
    );
  }
  ...
  vecToAzimElevDistance(vec: Vec3, azimElevDistance: Vec3) {
    ...
    const azim =
      Math.atan2(-vec.x / distance, -vec.z / distance) * math.RAD_TO_DEG;
...
class OrbitCameraInputTouch {
  ...
  onTouchMove(event: TouchEvent) {
    ...
    if (touches.length === 1) {
      ...
      aed.z -=
        diffInPinchDistance * this.distanceSensitivity * 0.1 * (aed.z * 0.1);
      this.orbitCamera.azimElevDistance.goto(aed);
